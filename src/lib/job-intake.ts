...
export type GapAnalysis = {
  ...
  gaps: GapItem[];
  languageGap: string | null;
  seniorityNote: string | null;
  overallNote: string;
};

// Como posicionar cada skill ausente com base no que o perfil tem
const positioningHints: Record<string, string> = {
  "Kubernetes":     "Experiência com Docker pode ser posicionada como base sólida para K8s; mencionar familiaridade com containerização.",
  "Go":             "Node.js e TypeScript mostram conforto com linguagens modernas; Go pode ser aprendido rapidamente.",
  "Java":           "Background full stack com TypeScript/Node.js demonstra polivalência; Java em contexto enterprise é transferível.",
  "Azure":          "Experiência com AWS cobre os fundamentos de cloud — providers são conceitualmente similares.",
  "GCP":            "Experiência com AWS demonstra maturidade em cloud; GCP compartilha os conceitos centrais.",
  "Terraform":      "CI/CD e automação de infraestrutura já estão no perfil — IaC é a próxima camada natural.",
  "Ansible":        "Automação e scripting fazem parte do background — Ansible é extensão direta.",
  "Kafka":          "Experiência com sistemas de alta disponibilidade e REST APIs demonstra entendimento de fluxos de dados.",
  "ElasticSearch":  "Observabilidade e análise de logs estão no perfil — ElasticSearch é ferramenta do mesmo domínio.",
  "GraphQL":        "REST APIs fazem parte do stack; GraphQL é evolução natural no mesmo paradigma.",
  "Microservices":  "Arquitetura de produção e design de sistemas estão no perfil — escopo microserviços é demonstrável.",
  "Vue.js":         "React no perfil demonstra domínio de framework reativo; Vue.js segue padrões similares.",
  "Angular":        "Experiência com React e TypeScript cobre os fundamentos; Angular usa conceitos equivalentes.",
  "Python":         "Automação e scripting já no perfil; Python pode ser destacado para scripts e tooling.",
  "C#":             "Background com TypeScript/Node facilita a transição; .NET tem APIs similares.",
  ".NET":           "Experiência full stack com tipagem forte (TypeScript) é base sólida para .NET.",
  "Linux":          "CI/CD, Docker e GitHub Actions pressupõem fluência em ambiente Linux.",
  "Rust":           "Interesse em sistemas confiáveis e performance é compatível com o perfil de engenharia.",
};

function getPositioningHint(skill: string): string {
  return positioningHints[skill] ??
    `${skill} não está declarado explicitamente, mas o perfil tem base técnica para aprender rapidamente no contexto do role.`;
}

export function analyzeGaps(job: ParsedJob, profile: CandidateProfile): GapAnalysis {
  const profileSkillSet = new Set(profile.coreStack.map((s) => s.toLowerCase()));
  const coveredSkills = job.skills.filter((s) => profileSkillSet.has(s.toLowerCase()));
  const missingSkills = job.skills.filter((s) => !profileSkillSet.has(s.toLowerCase()));

  // Classificar gaps por severidade
  const criticalSkills  = ["TypeScript", "JavaScript", "React", "Node.js", "SQL", "PostgreSQL"];
  const moderateSkills  = ["Docker", "CI/CD", "AWS", "Python", "REST APIs", "GitHub Actions"];

  const gaps: GapItem[] = missingSkills.map((skill) => {
    const severity =
      criticalSkills.includes(skill) ? "critical" :
      moderateSkills.includes(skill) ? "moderate" : "minor";

    return {
      skill,
      severity,
      positioning: getPositioningHint(skill),
    };
  });
  ...
  const languageGap = requiresGerman && !hasGerman
    ? "Alemão listado como requisito — não declarado no perfil. Considerar mencionar nível atual (mesmo que B1) na abordagem."
    : null;

  // Nota de senioridade
  const seniorityNote =
    /junior/i.test(job.seniority)
      ? "Vaga Junior com candidato Senior — pode gerar rejeição por overqualified. Adaptar abordagem para enfatizar interesse genuíno no escopo."
      : /staff|principal/i.test(job.seniority)
        ? "Vaga Staff/Principal — escopo de influência amplo. Destacar experiência de liderança técnica e impacto organizacional."
        : null;

  // Nota geral
  const criticalCount = gaps.filter((g) => g.severity === "critical").length;
  const overallNote =
    criticalCount === 0 && gaps.length <= 2
      ? "Cobertura técnica forte — gaps são gerenciáveis e o perfil central está bem alinhado."
      : criticalCount === 0
        ? "Stack principal coberta — gaps são tecnologias secundárias ou aprendíveis no contexto."
        : `${criticalCount} skill(s) crítica(s) ausente(s) — avaliar se a abordagem deve antecipar o tema ou focar nas forças.`;
